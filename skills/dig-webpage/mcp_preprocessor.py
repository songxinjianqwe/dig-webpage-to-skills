...
阶段一：过滤候选 reqid（输入：Claude 从 list_network_requests 返回值提取后写入的 JSON 文件）
    python mcp_preprocessor.py filter <requests_list_file> [-o candidate_reqids.json]
    输入格式：[{"reqid": 1, "method": "GET", "url": "...", "status": 200, "resourceType": "fetch"}, ...]
...
# ============================================================
# 阶段一：从请求摘要 JSON 中筛出候选 reqid
# ============================================================

def parse_request_list(text):
    """
    解析请求摘要 JSON 数组，返回请求摘要列表。

    输入格式（由 Claude 从 list_network_requests 返回值提取后用 Write 工具写入）：
    [
